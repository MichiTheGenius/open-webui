<script lang="ts">
	...
	const cloneHandler = async (func) => {
		...
		if (_function) {
			...
			goto('/openwebui/admin/functions/create');
		}
	};
	...
	<div class=" flex w-full space-x-2">
		...
		<div>
			<a
				...
				href="/openwebui/admin/functions/create"
			>
	...
	{#each filteredItems as func (func.id)}
		...
		>
			<a
				...
				href={`/openwebui/admin/functions/edit?id=${encodeURIComponent(func.id)}`}
			>
			...
			<div class="flex flex-row gap-0.5 self-center">
				...
				{:else}
					...
					<FunctionMenu
						{func}
						editHandler={() => {
							goto(`/openwebui/admin/functions/edit?id=${encodeURIComponent(func.id)}`);
						}}
						shareHandler={() => {
							shareHandler(func);
						}}

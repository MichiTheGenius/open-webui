<script lang="ts">
	...
	const cloneModelHandler = async (model) => {
		...
		goto('/openwebui/workspace/models/create');
	};
		...
		<div class=" flex flex-1 items-center w-full space-x-2">
			...
			<div>
				<a
					...
					href="/openwebui/workspace/models/create"
				>
			...
			>
				<div class="flex gap-4 mt-0.5 mb-0.5">
					...
					<a
						...
						href={`/openwebui/?models=${encodeURIComponent(model.id)}`}
					>
						...
						{:else}
							{#if $user?.role === 'admin' || model.user_id === $user?.id || model.access_control.write.group_ids.some( (wg) => group_ids.includes(wg) )}
								<a
									class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
									type="button"
									href={`/openwebui/workspace/models/edit?id=${encodeURIComponent(model.id)}`}
								>
									<svg
